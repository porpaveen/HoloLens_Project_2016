...
public class CustomMessages : Singleton<CustomMessages>
{
    ...
    public void SendChessTransform(string name)
    {
        // If we are connected to a session, broadcast our head info
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            // Create an outgoing network message to contain all the info we want to send
            NetworkOutMessage msg = CreateMessage((byte)TestMessageID.chess);

            AppendChess(msg, name);
    ...
    public void SendlookingTransform(string name)
    {
        // If we are connected to a session, broadcast our head info
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            // Create an outgoing network message to contain all the info we want to send
            NetworkOutMessage msg = CreateMessage((byte)TestMessageID.looking);

            Appendlooking(msg, name);
    ...
    void AppendChess(NetworkOutMessage msg, string name)
    {
        msg.Write(name);
    }

    void Appendlooking(NetworkOutMessage msg, string name)
    {
        msg.Write(name);
    }
